...
if (logger is null)
{
    WriteLine("logger is null, so creating default");
...
{
    logger.LogDebug("Logging is enabled");
} else {
    logger.LogDebug("Debug Logging is enabled, but it said it wasn't");
    logger.LogInformation("Debug is not enabled");
}
